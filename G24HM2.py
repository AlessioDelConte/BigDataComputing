...
def word_count1(sc, docs):
    ...
    # groupByKey().map(f2) has been replaced by reduceByKey, which also compute the shuffle
...
def word_count2_random(sc, docs, K):
    def f1(document):
        pairs_dict = {}
        for word in document.split(' '):
            if word not in pairs_dict.keys():
    ...
    def f3(grouped_key_value_list):
        ...
        for word, occ in list(grouped_key_value_list)[0]:   # Iterate through a single partition with key x = [0, k)
            if word not in pairs_dict.keys():
                pairs_dict[word] = occ
            else:
                pairs_dict[word] += occ
...
def word_count2_partitioned(sc, docs, K):
    def f1(documents):
        pairs_dict = {}
